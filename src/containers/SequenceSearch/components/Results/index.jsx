          ...
          this.props.jobId && (this.props.status === "loading" || this.props.status === "success" || this.props.status === "partial_success") && [
            <div className="row" key={`results-div`}>
              ...
              }
              {
                ...
                </div> : this.props.status === "loading" ? '' : this.props.filter ? <p className="mt-3">No results. Try a different search or press the Clear button to view all results.</p> : <p className="mt-2">The query sequence did not match any {this.props.databases} sequences.</p>
              }
            </div>
          ]
        }
